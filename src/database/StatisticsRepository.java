...
public class StatisticsRepository {
    ...
    // This method is used to provide a percentage statistic in the StatisticsViews
    // class.
    public int retrievePercentageAcquiredCertificates(Gender gender) {
        int percentageAcquiredCertificates = 0;
        try {
            PreparedStatement preparedStatement = this.dbConnection.getConnection().prepareStatement(
    ...
    // This method is used to retrieve the average progress in each course module
    // for a given course so it can be displayed on the StatisticsViews.
    // A List is used instead of a 2d int array because you cannot get the size of
    // the ResultSet without a 2nd query which increases complexity and possibility
    // of failure unnecessarily.
    ...
    public List<int[]> retrieveAverageProgressionPerModule(String courseName) {
        ...
        try {
            PreparedStatement statement = this.dbConnection.getConnection().prepareStatement(
                    "SELECT ContentID, AVG(Percentage) AS AverageProgression FROM Progress WHERE ContentID IN (SELECT ContentID FROM Course JOIN Module ON Course.CourseName = Module.CourseName WHERE Course.CourseName = ?) GROUP BY ContentID");
    ...
    // This method is used so the app user can check the progress of a given student
    // in a given course
    // A List of integer arrays is used instead of a 2d array since you can't get
    // the length of the ResultSet without either first putting it in a list or
    // using a 2nd query.
    ...
    public List<int[]> retrieveProgressionPerModule(String studentEmail, String courseName) {
        ...
        try {
            ...
            // the query will then select the ContentID from Progress where the student
            // email matches the given email
            // and where the contentID matches the contentID's returned by the sub query,
            // leaving you only with the progress of the student in a specific course.
            PreparedStatement preparedStatement = this.dbConnection.getConnection().prepareStatement(
                    " SELECT ContentID, Percentage FROM Progress WHERE StudentEmail = ? AND ContentID IN (SELECT ContentID FROM Module WHERE CourseName = ?)");
    ...
    // This method gets the 3 most watched webcasts and returns them so that this
    // statistic can be shown in the statistics view.
    ...
    public List<Webcast> retrieveTop3MostWatchedWebcasts() {
        ...
        try {
            // The query works by sorting all webcasts by their descending order of views
            // and then returning their ContentID.
    ...
    // This method is used so you can check which courses are recommended in the
    // gui.

    public List<Course> retrieveRecommendedCourses(String courseName) {
        ArrayList<Course> retrievedCourses = new ArrayList<>();
        // The query works by retrieving the records from the CourseRecommendation table
        // where CourseName matches the given course name
        ...
        try (PreparedStatement preparedStatement = this.dbConnection.getConnection().prepareStatement(query)) {
            ...
            while (result.next()) {
                retrievedCourses.add(new Course(result.getString("CourseName"), result.getString("Subject"),
                        result.getString("Description"), Difficulty.valueOf(result.getString("Difficulty"))));
            }
        } catch (Exception e) {
            e.printStackTrace();
            return null;
    ...
    public List<Certificate> retrieveStudentCertificates(String studentEmail) {
        ...
        try {
            PreparedStatement statement = this.dbConnection.getConnection().prepareStatement(
                    "SELECT Certificate.*, Student.Name, Enrollment.CourseName FROM Certificate JOIN Enrollment ON Certificate.EnrollmentID = Enrollment.ID JOIN Student ON Enrollment.Email = Student.Email WHERE Student.Email = ?");
            statement.setString(1, studentEmail);
            ResultSet result = statement.executeQuery();
            while (result.next()) {
                Certificate certificate = new Certificate(result.getInt("CertificateID"),
                        result.getString("CourseName"), result.getString("Name"), result.getInt("EnrollmentID"),
                        result.getString("EmployeeName"), result.getInt("Grade"));
    ...
    // Retrieves the top 3 courses by number of certificates gotten and the number
    // of certificates for that course.
    // The method is used by the StatisticsLogic class to print the most viewed
    // webcasts on the GUI
    ...
    public List<Course> retrieveTop3CoursesByNumberOfCertificates() {
        ...
        try {
            Statement statement = this.dbConnection.getConnection().createStatement();
            ResultSet result = statement.executeQuery(
                    "SELECT TOP 3 CourseName, COUNT(ID) AS NrOfCertificates FROM Enrollment  JOIN Certificate ON Enrollment.ID = Certificate.EnrollmentID GROUP BY CourseName ORDER BY NrOfCertificates DESC");
            while (result.next()) {
    ...
    public int retrieveNumberOfStudentsWhoCompletedCourse(String courseName) {
        try {
            PreparedStatement statement = this.dbConnection.getConnection().prepareStatement(
                    "SELECT COUNT(Email) AS NumberOfStudents FROM Student WHERE Email IN (SELECT StudentEmail FROM Progress WHERE ContentID IN (SELECT ContentID FROM Module  WHERE CourseName = ?) GROUP BY StudentEmail HAVING SUM(Percentage)/COUNT(ContentID) = 100)");
            statement.setString(1, courseName);
            ResultSet result = statement.executeQuery();
            while (result.next()) {
