...
public class ModuleRepository extends Repository {
    ...
    public void update(Object domainObject) {
        if (domainObject instanceof Module) {
            ...
            try {
                ...
                statement.executeUpdate("UPDATE Module SET ContactEmail = '" + contactEmail + "', PositionInCourse = " + trackingNumber + " WHERE ContentID = " + contentID + "");
            } catch (SQLException e) {
                e.printStackTrace();
            }
        }
    ...
    public Module retrieveModuleByID(int ID) {
        ...
        try {
            statement = this.connection.getConnection().createStatement();
            ResultSet contactRetrieve = statement.executeQuery("SELECT * FROM Contact WHERE Email = '"+emailAddress+"'");
        ...
        Module module = new Module(date, status, title, version, positionWithinCourse, description, contactName, emailAddress);
        return module;
        } catch (SQLException e) {
            e.printStackTrace();
            return null;
